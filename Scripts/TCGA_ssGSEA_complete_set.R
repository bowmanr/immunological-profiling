...
mergedlist <- galon_engler; keyword <- "immune"
mergedlist <- c(noBlank, finalset); keyword <- 'macrophage'
...
name <- "ssGSEA_macrophage_Module_RNASeqCounts.RData" #Name of results, disease added in save function
...
for(x in diseases){
  ...
  for(y in 1:length(GSE)){
        exprs <- grep("exprs.RData", list.files(GSE[y], full.names=T), value=T)
        if(length(exprs)==0){next}
        load(exprs, verbose=T)
        
        name <- strsplit(GSE[y], split="/")
        name <- paste(name[[1]][3],keyword, ".RData", sep="_")
        exprs_1 <- exprs_1[!apply(exprs_1,1,function(p) any(is.na(p))),]
