...
# TCGA-Assembler Version 1.0.2 Module B
...
ExtractTissueSpecificSamples<-function(inputData, tissueType, singleSampleFlag, sampleTypeFile = "./SupportingFiles/TCGASampleType.txt")
{
  ...
  sampleType = read.table(file = sampleTypeFile, header = TRUE, sep = "\t", na.string = "", stringsAsFactors = FALSE);  
...
ProcessCNAData<-function(inputFilePath, outputFileName, outputFileFolder, refGenomeFile)
{
  ...
  RefGenome = read.table(file = refGenomeFile, header = TRUE, sep = "\t", na.string = "", stringsAsFactors = FALSE);
  NumGene = dim(RefGenome)[1];
  mode(RefGenome[, "txStarts"]) = "numeric";
  mode(RefGenome[, "txEnds"]) = "numeric";

  # Load copy number data
  InData = read.table(file = inputFilePath, header = TRUE, sep = "\t", na.string = "", stringsAsFactors = FALSE, check.names = FALSE);
