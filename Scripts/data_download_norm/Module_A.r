...
# TCGA-Assembler Version 1.0.2 Module A
...
DownloadCNAData <- function(traverseResultFile, saveFolderName, cancerType, assayPlatform = "genome_wide_snp_6", inputPatientIDs = NULL, outputFileName = "")
{
  options(warn=-1);
...
DownloadMethylationData <- function(traverseResultFile, saveFolderName, cancerType, assayPlatform, inputPatientIDs = NULL, outputFileName = "")
{
  options(warn=-1);
...
DownloadRNASeqData <-function (traverseResultFile, saveFolderName, cancerType, assayPlatform, dataType, inputPatientIDs = NULL, outputFileName = "")
{
  options(warn=-1);
  
  writeLines("**********************************************************************************");
  writeLines("");
  writeLines(paste("Download RNA-seq data of ", cancerType, " patients.", sep = ""));
...
DownloadRPPAData <- function(traverseResultFile, saveFolderName, cancerType, assayPlatform = "mda_rppa_core", inputPatientIDs = NULL, outputFileName = "") 
{
  options(warn=-1);
  ...
  replaceTable = cbind(wrongSymbol = c("ACACAACACB", "AKT1AKT2 AKT3", "GSK3AGSK3B", "MAPK1MAPK3", "RAB11ARAB11B"), 
                      correctSymbol = c("ACACA ACACB", "AKT1 AKT2 AKT3", "GSK3A GSK3B", "MAPK1 MAPK3", "RAB11A RAB11B"));
  ...
  IX1 = order(toupper(left_columns), decreasing = FALSE);
  IX2 = order(toupper(Annotation[, 2]), decreasing = FALSE);
  if (sum(left_columns[IX1] != Annotation[IX2, 2]) > 0)
  {
    writeLines("Downloaded data have antibody IDs not consistent with the antibody annotations.");    
  }
  left_columns = left_columns[IX1];
  data = data[IX1, , drop = FALSE];
  Annotation = Annotation[IX2, , drop = FALSE];
  left_columns = paste(Annotation[, 1], "|", Annotation[, 2], sep = "");  
...
DownloadmiRNASeqData <- function(traverseResultFile, saveFolderName, cancerType, assayPlatform = "miRNASeq", inputPatientIDs = NULL, outputFileName = "")
{
  options(warn=-1);
