...
#### Example
##############
source("/Volumes/MacintoshHD3/data/TCGA_Assembler/Scripts/estimate_ssGSEA_BB.r")
 load("/Users/bowmanr/Projects/Imm_profile/Datasets/Gene_sets/Galon_Engler_list.RData",verbose=TRUE)
 load("/Volumes/MacintoshHD3/data/TCGA_Assembler/Data/Merged/All_RNASeqCounts.RData",verbose=TRUE)

SI_geneset <- galon_engler #list of gene_sets
    exprs <- bigmat[,1:10] #gene expression matirx
   test<- exprs[apply(exprs,1,function(x){any(x==0)}),]
scores <- estimate_ssGSEA_BB(exprs=exprs,SI_geneset=SI_geneset)
