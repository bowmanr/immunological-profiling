...
for(x in diseases){
		...
		load(paste(".", x, "RNASeq", "RNASeqCounts.RData",sep="/"),verbose=TRUE)
		...
		pdf(file=paste("./", x, "/Plots/", "/cytokines_violin.pdf",sep=""))
		par(mfrow=c(2,2))
		...
		for(i in 1:length(merged)){
			...
			cols <- c("#9E0142", "#D53E4F", "#F46D43", "#FDAE61", "#FEE08B", "#FFFFBF", "#E6F598", "#ABDDA4", "#66C2A5", "aquamarine4","#3288BD", muted("dodgerblue2"), "#5E4FA2", muted("blueviolet"))[color_index]
		  	...
		  	boxplot(merged[[i]][,"RNA"]~factor(merged[[i]][,"CNA"]), main=names(merged)[i], xlab="Copy Number Alteration (GISTIC)",ylab="RNASeq Counts", col=cols)
